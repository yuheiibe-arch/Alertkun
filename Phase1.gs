 ...
 * ★UPDATE: 募集シフトを休館日アサインエラーから完全除外（「募集先生」エラーの防止）
 * ==========================================
 */
function runShiftCheckerPhase1() {
  ...
  const loadShifts = (sheet, sourceName) => {
    ...
    const cols = getColumnIndices(data[0], ['医籍番号', '名前', 'クリニックNo', 'クリニック名', '診療科', '勤務日', '勤務開始時間', '勤務終了時間']);
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      const clinicId = cols['クリニックNo'] !== -1 ? String(row[cols['クリニックNo']]).trim() : "";
      ...
      unifiedShifts.push({ sourceSheet: sourceName, dateObj, dateStr, displayDate, clinic: rawClinic, normClinic, clinicId, dept, doctor: docClean, docId: docId, startStr, endStr, startMin, endMin });

      if (closedDataMap.has(`${dateStr}_${normClinic}`)) {
        // ★修正: 「募集」シフトの場合は、Phase 1の「アサインエラー」から完全除外する（募集先生アラート防止）
        if (sourceName === "募集" || docClean === "募集") {
           // 完全スルー（時給が入っているかどうかの監査は Phase 3 で行う）
        } else {
  ...
  loadShifts(shiftSs.getSheetByName("募集シフト"), "募集"); 
  
  const groupedShifts = {};
  unifiedShifts.forEach(shift => {
    if (shift.sourceSheet === "募集") return; 
    if (isNaN(shift.startMin) || isNaN(shift.endMin)) return;
    if (shift.dateObj < today) return; 

    if (twoDocRequestsMap.has(`${shift.dateStr}_NAME_${shift.normClinic}`) || twoDocRequestsMap.has(`${shift.dateStr}_ID_${shift.clinicId}`)) {
      return; 
    }
