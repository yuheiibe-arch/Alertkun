...
function runShiftCheckerPhase4() {
  ...
  // --- ★ NEW: タイムラグ考慮（2日連続検知）用システムシートの準備 ---
  const SYSTEM_SHEET_NAME = "SystemData_Phase4Lag";
  ...
  let previousDelayData = {};
  const sysVal = systemSheet.getRange(1, 1).getValue();
  if (sysVal) {
    try {
      const parsed = JSON.parse(sysVal);
      if (typeof parsed === 'object') previousDelayData = parsed;
    } catch(e) {}
  }
  const currentDelayData = {};

  // エラー判定を「即時出力」ではなく「2日連続なら出力」に変換するラッパー関数
  const processLagError = (type, displayDate, clinic, dept, doctor, empType, workTime, errorDetail, actionMsg, uniqueId, targetUrl = null) => {
    const firstDetectedDate = previousDelayData[uniqueId];
    if (!firstDetectedDate) {
      // 初回検知：システムシートにメモするだけ（当日の編集猶予）
      currentDelayData[uniqueId] = todayYYYYMMDD;
    } else {
      // 継続検知：以前の日付を引き継ぐ
      currentDelayData[uniqueId] = firstDetectedDate;
      // 初回検知が「昨日以前」ならタイムラグ猶予終了としてエラー出力
      if (firstDetectedDate !== todayYYYYMMDD) {
        addError(type, displayDate, clinic, dept, doctor, empType, workTime, errorDetail, actionMsg, uniqueId, targetUrl);
      }
    }
  };
  ...
  // ★ 最新の検知リスト（タイムラグ記録）をシステムシートに保存
  systemSheet.getRange(1, 1).setValue(JSON.stringify(currentDelayData));
